  ...
  if (template === "og-card") {
    return [
      ...
      },
      {
        id: "l2",
        kind: "heading",
        text: "Northwind · 4.2× AI Overview citations",
        x: 6,
        y: 22,
        size: 56,
        weight: 600,
      ...
      },
      {
        id: "l3",
        kind: "sub",
        text: "How we rebuilt content for the new search game.",
        x: 6,
        y: 60,
        size: 22,
        weight: 400,
  ...
  if (template === "case-cover") {
    return [
      ...
      },
      {
        id: "l3",
        kind: "sub",
        text: "+312% organic signups in 6 months.",
        x: 6,
        y: 60,
        size: 28,
        weight: 400,
  ...
  if (template === "ad-square") {
    return [
      ...
      },
      {
        id: "l3",
        kind: "sub",
        text: "BAZ ties its fee to revenue.",
        x: 8,
        y: 76,
        size: 22,
        weight: 400,
    ...
    return [
      {
        id: "l1",
        kind: "heading",
        text: '"BAZ moved the needle in the first 30 days — and told us when our targets were unrealistic."',
        x: 8,
        y: 18,
        size: 44,
        weight: 500,
      ...
      },
      {
        id: "l2",
        kind: "sub",
        text: "Margaret H., Head of Marketing, Northwind",
        x: 8,
        y: 80,
        size: 20,
        weight: 400,
...
const bgFor = (template: TemplateId, brand: BrandKit): string => {
  ...
  if (template === "case-cover")
    return `linear-gradient(135deg, ${brand.ink} 0%, #1a1a1f 60%, ${brand.accent} 130%)`;
  if (template === "ad-square")
...
export default function CanvaPage() {
  ...
  const exportPng = async () => {
    ...
    try {
      const mod = await import("html-to-image").catch(() => null) as typeof import("html-to-image") | null;
      if (mod) {
    ...
    <div className="container mx-auto py-10 md:py-16">
      <header className="flex flex-wrap items-end justify-between gap-6 mb-8">
        <div>
          <p className="font-mono uppercase tracking-[0.18em] text-[11px] text-accent mb-2">
            BAZ Canva · /admin/canva
          </p>
          <h1 className="font-display text-display-xl font-medium tracking-[-0.035em]">
            Design assets, in-house.
          </h1>
          <p className="mt-3 text-muted-foreground max-w-xl">
            Generate OG cards, case covers, ads, and quotes with your brand kit. No third-party
            tool. Export PNG or SVG.
          </p>
        </div>
      ...
      <div className="grid lg:grid-cols-[260px_1fr_280px] gap-6">
        ...
        <aside className="rounded-2xl border border-border bg-background p-3">
          <p className="font-mono uppercase tracking-[0.18em] text-[11px] text-muted-foreground/60 px-2 pb-2">
            Templates
          </p>
          ...
          <p className="font-mono uppercase tracking-[0.18em] text-[11px] text-muted-foreground/60 px-2 pt-6 pb-2">
            Brand kit
          </p>
          <div className="space-y-2 px-2">
            <Field
              ...
              type="color"
              value={brand.primary}
              onChange={(v) => setBrand({ ...brand, primary: v })}
            />
            <Field
              label="Accent"
              type="color"
            ...
            <Field
              ...
              onChange={(v) => setBrand({ ...brand, displayFont: v })}
            />
          </div>
        </aside>
            ...
            {layers.map((l) => (
              <div
                ...
                style={{
                  ...
                  fontFamily:
                    l.kind === "heading" || l.kind === "badge" ? brand.displayFont : brand.bodyFont,
                  fontSize: `${l.size}px`,
                  fontWeight: l.weight,
                  color: l.color,
                  textAlign: l.align,
                  lineHeight: 1.05,
                  letterSpacing:
                    l.kind === "heading" ? "-0.025em" : l.kind === "badge" ? "0.18em" : "normal",
                  whiteSpace: "pre-line",
                  maxWidth: "88%",
                }}
              >
                {l.kind === "badge" ? (
                  <span style={{ textTransform: "uppercase", fontSize: l.size }}>{l.text}</span>
                ) : (
                  l.text
                )}
              </div>
        ...
        <aside className="rounded-2xl border border-border bg-background p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              type="button"
              onClick={addLayer}
              className="text-xs font-medium px-3 py-1.5 rounded-full bg-primary text-foreground hover:bg-primary/90"
            >
              + Add
            </button>
          </div>
          <ul className="space-y-1.5 mb-4">
            {layers.map((l) => (
              <li
                key={l.id}
                className={`flex items-center justify-between rounded-xl px-3 py-2 text-sm border ${selected === l.id ? "border-accent bg-white dark:bg-zinc-900" : "border-border bg-white dark:bg-zinc-900"}`}
              >
                <button
                  type="button"
                ...
                >
                  <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground/60 mr-2">
                    {l.kind}
                  </span>
                ...
                <button
                  ...
                  className="ml-2 text-muted-foreground/60 hover:text-accent"
                >
                  ×
                </button>
              </li>
            ...
            <div className="space-y-3 border-t border-border pt-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <NumField
                  label="Size"
                  value={sel.size}
                  onChange={(v) => updateLayer(sel.id, { size: v })}
                />
                <NumField
                  label="Weight"
                  value={sel.weight}
                  onChange={(v) => updateLayer(sel.id, { weight: v })}
                />
                <NumField
                  label="X %"
                  value={sel.x}
                  onChange={(v) => updateLayer(sel.id, { x: v })}
                />
                <NumField
                  label="Y %"
                  value={sel.y}
                  onChange={(v) => updateLayer(sel.id, { y: v })}
                />
              </div>
              <label className="block">
          ...
          {!sel && (
            <p className="text-xs text-muted-foreground border-t border-border pt-4">
              Click any layer on the canvas to edit it, or add a new one. Use the brand kit on the
              left to set colors and fonts across all templates.
            </p>
          )}
        </aside>
      </div>
...
function Field({
  ...
  return (
    <label className="flex items-center gap-2 text-xs">
      <span className="w-20 text-muted-foreground font-mono uppercase tracking-[0.14em] text-[10px]">
        {label}
      </span>
      {type === "color" ? (
        <input
          ...
          className="w-10 h-7 rounded border border-border bg-white dark:bg-zinc-900"
        />
      ) : (
        <input
          type="text"
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className="flex-1 rounded border border-border bg-white dark:bg-zinc-900 px-2 py-1 text-xs"
        />
      )}
    </label>
  );
